...
class BatchDTO(BaseModel):
    ...
    available_liters: float | None = Field(None, ge=0)   # TODO: implement available_liters = volume_liters - sum(consumed)
    deleted_at: Annotated[str | None, Field(None, pattern=settings.string_patterns.deleted_at)]   # TODO: implement available_liters = volume_liters - sum(consumed)
    ...
    def check_batch_timestamps(self):
        ...
        code_date = datetime.datetime.strptime(self.batch_code[4:12], settings.datetime_string_formats.batch_code)
        received_at_date = datetime.datetime.strptime(self.received_at, settings.datetime_string_formats.received_at)
